...
This Rails plugin gives you the ability to transparently cache single active record objects using memcached.
...
In your model:
  ...
  # you should use the :logical_delete option if the method to verify a record is deleted
  # is different than :deleted?, disregard if not using logical deletes
  # so out of the box the plugin works with destroys and with logical deletes
